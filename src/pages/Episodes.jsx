    ...
    <div className="page-transition" style={{ paddingTop: '120px', background: '#000', minHeight: '100vh', paddingBottom: '100px' }}>
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
          {episodes.map(ep => (
            <Link to={`/episodes/${ep.id}`} key={ep.id} style={{ textDecoration: 'none' }}>
              <div style={{ border: '1px solid var(--gold-dim)', padding: '24px', background: 'rgba(20,20,20,0.8)', transition: 'transform 0.3s', cursor: 'pointer' }} onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}>
